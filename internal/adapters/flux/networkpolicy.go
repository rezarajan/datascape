...
// emitNetworkPolicies compiles the three-object deny floor for stackName
// (week-four plan, Enforcement round → Revision 3): default-deny-egress
// (a), allow-cluster-egress (b: DNS, istiod, same-namespace, HBONE), and
// — only when a waypoint actually exists in this stack (waypointPresent,
// threaded from emitEgress's own return so this file never references a
// pod selector matching nothing, golden rule 24's "no dangling edge"
// spirit) — allow-waypoint-egress (c): the mesh's sole external gate,
// left otherwise unrestricted because it is already guarded by the
// compiled ServiceEntry/AuthorizationPolicy allow-list (egress.go), the
// authoritative identity layer this NetworkPolicy floor is deliberately
// not duplicating (golden rule 50).
func emitNetworkPolicies(files map[string][]byte, stackName string, waypointPresent bool) error {
